...
- Древовидный режим (как файловый менеджер): ветки можно сворачивать/разворачивать
- Fallback в плоский список для источников без файлового пути
...
- Для `Shizuku Android/` нужно:
  ...
  4. Если Shizuku запущен в режиме `shell` (ADB), на некоторых прошивках доступ к `Android/data` частичный.
     Для полного доступа нужен backend `root`/Sui.
